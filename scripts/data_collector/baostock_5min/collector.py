...
class BaostockNormalizeHS3005min(BaseNormalize):
    ...
    def __init__(
        ...
        qlib_data_1d_dir: str, Path
            the qlib_bak data to be updated for yahoo, usually from: Normalised to 5min using local 1d data
        date_field_name: str
        ...
        """
        bs.login()
        qlib_bak.init(provider_uri=qlib_data_1d_dir)
...
class Run(BaseRun):
    ...
    def download_data(
        ...
        ---------
            # get hs300 5min data
            $ python collector.py download_data --source_dir ~/.qlib_bak/stock_data/source/hs300_5min_original --start 2022-01-01 --end 2022-01-30 --interval 5min --region HS300
        """
    ...
    def normalize_data(
        ...
        qlib_data_1d_dir cannot be None, normalize 5min needs to use 1d data;
            ...
            qlib_data_1d can be obtained like this:
                $ python scripts/get_data.py qlib_data --target_dir ~/.qlib_bak/qlib_data/cn_data --interval 1d --region cn --version v3
            or:
                download 1d data, reference: https://github.com/microsoft/qlib/tree/main/scripts/data_collector/yahoo#1d-from-yahoo

        Examples
        ---------
            $ python collector.py normalize_data --qlib_data_1d_dir ~/.qlib_bak/qlib_data/cn_data --source_dir ~/.qlib_bak/stock_data/source/hs300_5min_original --normalize_dir ~/.qlib_bak/stock_data/source/hs300_5min_nor --region HS300 --interval 5min
        """
        if qlib_data_1d_dir is None or not Path(qlib_data_1d_dir).expanduser().exists():
            raise ValueError(
                "If normalize 5min, the qlib_data_1d_dir parameter must be set: --qlib_data_1d_dir <user qlib_bak 1d data >, Reference: https://github.com/microsoft/qlib/tree/main/scripts/data_collector/yahoo#automatic-update-of-daily-frequency-datafrom-yahoo-finance"
            )

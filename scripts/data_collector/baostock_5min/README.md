...
### Get Qlib data(`bin file`)
  ...
  - examples:
    ```bash
    # hs300 5min
    python scripts/get_data.py qlib_data --target_dir ~/.qlib_bak/qlib_data/hs300_data_5min --region hs300 --interval 5min
    ```
...
> If the above ready-made data can't meet users' requirements,  users can follow this section to crawl the latest data and convert it to qlib-data.
  ...
  3. dump data: `python scripts/dump_bin.py dump_all`
     ...
     - examples:
       ```bash
       # dump 5min cn
       python dump_bin.py dump_all --csv_path ~/.qlib_bak/stock_data/source/hs300_5min_nor --qlib_dir ~/.qlib_bak/qlib_data/hs300_5min_bin --freq 5min --exclude_fields date,symbol
       ```
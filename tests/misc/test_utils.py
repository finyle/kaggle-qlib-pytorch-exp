...
from qlib_bak import init
from qlib_bak.config import C
from qlib_bak.log import TimeInspector
from qlib_bak.constant import REG_CN, REG_US, REG_TW
from qlib_bak.utils.time import cal_sam_minute as cal_sam_minute_new, get_min_cal, CN_TIME, US_TIME, TW_TIME
...
def cal_sam_minute(x: pd.Timestamp, sam_minutes: int, region: str):
    ...
    if 0 <= minute_index < 120 or region_time != CN_TIME:
        ...
        raise ValueError("calendar minute_index error, check `min_data_shift` in qlib_bak.config.C")

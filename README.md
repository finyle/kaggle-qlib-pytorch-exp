...
* Clone the repository and install ``Qlib`` as follows.
    ```bash
    git clone https://github.com/microsoft/qlib.git && cd qlib_bak
...
mkdir -p ~/.qlib_bak/qlib_data/cn_data
...
### Get with module
  ```bash
  # get 1d data
  python -m qlib_bak.run.get_data qlib_data --target_dir ~/.qlib_bak/qlib_data/cn_data --region cn

  # get 1min data
  python -m qlib_bak.run.get_data qlib_data --target_dir ~/.qlib_bak/qlib_data/cn_data_1min --region cn --interval 1min
...
- Run the initialization code and get stock data:
  ...
  # Initialization
  mount_path = "~/.qlib_bak/qlib_data/cn_data"  # target_dir
  qlib_bak.init(mount_path=mount_path, region=REG_CN)
...
1. Quant Research Workflow: Run  `qrun` with lightgbm workflow config ([workflow_config_lightgbm_Alpha158.yaml](examples/benchmarks/LightGBM/workflow_config_lightgbm_Alpha158.yaml) as following.
    ```bash
      cd examples  # Avoid running program under the directory contains `qlib_bak`
      qrun benchmarks/LightGBM/workflow_config_lightgbm_Alpha158.yaml
    ```
    If users want to use `qrun` under debug mode, please use the following command:
    ```bash
    python -m pdb qlib_bak/workflow/cli.py examples/benchmarks/LightGBM/workflow_config_lightgbm_Alpha158.yaml
    ```
...
| --                                         | --        | --           |
| [Alpha360](qlib_bak/contrib/data/handler.py) |  √        |  √           |
| [Alpha158](qlib_bak/contrib/data/handler.py) |  √        |  √           |

...
VERSION = get_version("qlib_bak/__init__.py")
...
REQUIRED = [
    ...
    # and we need to verify whether version 2.0 of mlflow can serve qlib_bak properly.
    ...
    # which would cause qlib_bak.workflow.cli to not work properly,
...
extensions = [
    Extension(
        "qlib_bak.data._libs.rolling",
        ["qlib_bak/data/_libs/rolling.pyx"],
        language="c++",
        include_dirs=[NUMPY_INCLUDE],
    ),
    Extension(
        "qlib_bak.data._libs.expanding",
        ["qlib_bak/data/_libs/expanding.pyx"],
        language="c++",
        include_dirs=[NUMPY_INCLUDE],
    ),
]
...
setup(
    ...
    entry_points={
        ...
        "console_scripts": [
            "qrun=qlib_bak.workflow.cli:run",
        ],
    },
    ...
    extras_require={
        "dev": [
            ...
            # the error "multiprocessing.Value()" is detected in the file "qlib_bak/rl/utils/data_queue.py",
            ...
            # this version, causes qlib_bak installation to fail, so we've limited the scs version a bit for now.
            "scs<=3.2.4",
            "beautifulsoup4",
